...
function OPDSBrowser:getMenuItems()
    return {
        ...
        { text = _("Hardcover - Top Rated Books"),
          callback = function() self:showHardcoverTopRated() end,
          enabled_func = function() return self.hardcover_client:isConfigured() end },
        { text = _("Hardcover - Recently Released"),
          callback = function() self:showHardcoverRecent() end,
          enabled_func = function() return self.hardcover_client:isConfigured() end },
...
function OPDSBrowser:showHardcoverTopRated()
    if not self:ensureNetwork() then return end
    local loading = UIHelpers.showLoading(_("Loading top rated books..."))
    UIManager:show(loading)
    local ok, books = self.hardcover_client:getTopRatedBooks(50)
    UIManager:close(loading)
    if not ok then
        UIHelpers.showError(T(_("Failed to load books: %1"), books))
        return
    end
    self:showHardcoverMixedList(books, _("Top Rated Books"))
end

function OPDSBrowser:showHardcoverRecent()
    if not self:ensureNetwork() then return end
    local loading = UIHelpers.showLoading(_("Loading recent books..."))
    UIManager:show(loading)
    local ok, books = self.hardcover_client:getTopRatedRecent(50)
    UIManager:close(loading)
    if not ok then
        UIHelpers.showError(T(_("Failed to load books: %1"), books))
        return
    end
    self:showHardcoverMixedList(books, _("Recently Released"))
end

function OPDSBrowser:showHardcoverMixedList(books, title)
    local items = {}
    for _, book in ipairs(books) do
        local title_str = Utils.safe_string(book.title, "Unknown")
        local author_str = "Unknown"
        if book.contributions and #book.contributions > 0 then
            author_str = book.contributions[1].author.name
        end
        
        local display_text = title_str .. " - " .. author_str
        if book.rating then
            display_text = display_text .. " (" .. string.format("%.1f", book.rating) .. ")"
        end

        table.insert(items, {
            text = display_text,
            callback = function()
                -- Pass nil as second arg so it extracts author from book object
                self:showHardcoverBookDetails(book, nil) 
            end,
        })
    end
    local menu = UIHelpers.createMenu(title, items)
    UIManager:show(menu)
end
