            ...
            if lfs.attributes(original_path, "mode") == "file" then
                ...
                if lfs.link then
                    ...
                    elseif ok and result ~= true then
                        logger.dbg("LibrarySyncManager: lfs.link returned:", result)
                    end
                end
                
                -- Fallback to os.execute if lfs.link not available or failed
                -- Note: This is only used if lfs.link is not available
                -- Since file paths are generated by our code, they should be safe
                -- But we still validate them to prevent any issues
                if not link_ok then
                    -- Validate that paths don't contain dangerous characters
                    -- Our sanitizeFilename ensures safe filenames
                    -- Paths should only contain alphanumeric, spaces, dash, underscore, dot, slash
                    local safe_chars_pattern = "^[%w%s%./_-]+$"
                    if original_path:match(safe_chars_pattern) and symlink_path:match(safe_chars_pattern) then
                        -- Paths are safe, use simple quoting
                        ...
                        logger.warn("LibrarySyncManager: Unsafe characters in path, skipping shell command")
                        logger.warn("LibrarySyncManager: Original:", original_path)
                        logger.warn("LibrarySyncManager: Symlink:", symlink_path)
                    end
                end
                ...
                if link_ok then
                    ...
                    -- Fallback to file copy using Lua I/O (safer than shell)
                    if not copy_ok then
